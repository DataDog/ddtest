...
func TestGetCITagsSkipsGitEnrichmentInPayloadFilesMode(t *testing.T) {
	...
	applyEnvironmentalDataIfRequiredFunc = func(tags map[string]string) {
		...
		tags[git.GitRepositoryURL] = "https://example.com/repo-from-env.git"
		tags[git.GitCommitSHA] = "commit-sha-from-env"
		tags[git.GitBranch] = "main-from-env"
		tags[git.GitCommitMessage] = "commit-message-from-env"
		tags["env.applied"] = "true"
	}
	...
	assert.Equal(t, "https://example.com/repo-from-env.git", tags[git.GitRepositoryURL])
	assert.Equal(t, "commit-sha-from-env", tags[git.GitCommitSHA])
	assert.Equal(t, "main-from-env", tags[git.GitBranch])
	assert.Equal(t, "commit-message-from-env", tags[git.GitCommitMessage])
	assert.NotContains(t, tags, git.GitHeadMessage)
	assert.Equal(t, "true", tags["env.applied"])
...
func TestGetCITagsAddsBazelProviderInManifestModeWithoutProvider(t *testing.T) {
	...
	t.Setenv(manifestFilePathEnv, "workspace/.testoptimization/manifest.txt")
