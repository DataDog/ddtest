...
func TestGetEnvironmentalData_NoFile(t *testing.T) {
	t.Setenv(constants.TestOptimizationEnvironmentDataFilePath, "")
...
func TestGetEnvironmentalData_UsesEnvVar(t *testing.T) {
	origEnv := os.Getenv(constants.TestOptimizationEnvironmentDataFilePath)
	defer func() { _ = os.Setenv(constants.TestOptimizationEnvironmentDataFilePath, origEnv) }()
	...
	if err := os.Setenv(constants.TestOptimizationEnvironmentDataFilePath, customPath); err != nil {
		t.Fatal(err)
	}
...
func TestGetEnvDataFileName_WithEnvVar(t *testing.T) {
	...
	orig := os.Getenv(constants.TestOptimizationEnvironmentDataFilePath)
	defer func() { _ = os.Setenv(constants.TestOptimizationEnvironmentDataFilePath, orig) }()
...
func TestGetEnvDataFileName_WithoutEnvVar(t *testing.T) {
	origEnv := os.Getenv(constants.TestOptimizationEnvironmentDataFilePath)
	defer func() { _ = os.Setenv(constants.TestOptimizationEnvironmentDataFilePath, origEnv) }()
	_ = os.Setenv(constants.TestOptimizationEnvironmentDataFilePath, "")
...
func TestApplyEnvironmentalDataIfRequired_NoEnvFile(t *testing.T) {
	...
	tags := map[string]string{
		...
		git.GitBranch:               "",
		git.GitTag:                  "",
		git.GitCommitAuthorDate:     "",
		git.GitCommitAuthorName:     "",
		git.GitCommitAuthorEmail:    "",
		git.GitCommitCommitterDate:  "",
		git.GitCommitCommitterName:  "",
		git.GitCommitCommitterEmail: "",
		git.GitCommitMessage:        "",
...
func TestApplyEnvironmentalDataIfRequired_WithEnvFile(t *testing.T) {
	...
	if tags[git.GitRepositoryURL] != "repo_url" {
		t.Errorf("Expected GitRepositoryURL to be 'repo_url', got '%s'", tags[git.GitRepositoryURL])
	}
	if tags[git.GitCommitSHA] != "sha" {
		t.Errorf("Expected GitCommitSHA to be 'sha', got '%s'", tags[git.GitCommitSHA])
	}
	if tags[git.GitBranch] != "branch" {
		t.Errorf("Expected GitBranch to be 'branch', got '%s'", tags[git.GitBranch])
	}
	if tags[git.GitTag] != "tag" {
		t.Errorf("Expected GitTag to be 'tag', got '%s'", tags[git.GitTag])
	}
	if tags[git.GitCommitAuthorDate] != "date1" {
		t.Errorf("Expected GitCommitAuthorDate to be 'date1', got '%s'", tags[git.GitCommitAuthorDate])
	}
	if tags[git.GitCommitAuthorName] != "author" {
		t.Errorf("Expected GitCommitAuthorName to be 'author', got '%s'", tags[git.GitCommitAuthorName])
	}
	if tags[git.GitCommitAuthorEmail] != "author@example.com" {
		t.Errorf("Expected GitCommitAuthorEmail to be 'author@example.com', got '%s'", tags[git.GitCommitAuthorEmail])
	}
	if tags[git.GitCommitCommitterDate] != "date2" {
		t.Errorf("Expected GitCommitCommitterDate to be 'date2', got '%s'", tags[git.GitCommitCommitterDate])
	}
	if tags[git.GitCommitCommitterName] != "committer" {
		t.Errorf("Expected GitCommitCommitterName to be 'committer', got '%s'", tags[git.GitCommitCommitterName])
	}
	if tags[git.GitCommitCommitterEmail] != "committer@example.com" {
		t.Errorf("Expected GitCommitCommitterEmail to be 'committer@example.com', got '%s'", tags[git.GitCommitCommitterEmail])
	}
	if tags[git.GitCommitMessage] != "message" {
		t.Errorf("Expected GitCommitMessage to be 'message', got '%s'", tags[git.GitCommitMessage])
	}
