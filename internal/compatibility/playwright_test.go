...
func TestPlaywrightFastAdapterIntegration(t *testing.T) {
...
`, projects)
	...
	configureFramework(shellCommand(baseCommand...), "apps/web/tests/{a.spec,b.test}.ts")
	...
	configureFramework(shellCommand(emptyCommand...), "apps/web/tests/__ddtest_no_match__")
	...
	configureFramework(shellCommand(brokenCommand...), "apps/web/tests/broken.spec.ts")
	brokenPlaywright := framework.NewPlaywright()
	brokenFiles, err := brokenPlaywright.DiscoverTestFiles(ctx, discovery.TestFileSet{})
	if err != nil {
		t.Fatal(err)
	}
	requireFiles(t, brokenFiles, []string{"apps/web/tests/broken.spec.ts"})
	if err := brokenPlaywright.RunTests(ctx, brokenFiles, nil); err == nil {
		t.Fatal("collection failure must be reported during execution")
	}
}
