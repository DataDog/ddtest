...
func isGitFound() bool {
	gitFinderOnce.Do(func() {
		...
		if err != nil {
			slog.Debug("civisibility.git: git executable not found")
		}
	})
...
func getSafeDirectoryConfig() string {
	safeDirectoryOnce.Do(func() {
		...
		if err != nil {
			slog.Debug("civisibility.git: error getting current working directory for safe.directory")
			return
		}

		gitDir, err := getParentGitFolder(currentDir)
		if err != nil || gitDir == "" {
			slog.Debug("civisibility.git: could not find git folder for safe.directory")
			return
		}
		...
		slog.Debug("civisibility.git: using safe.directory config", "path", safeDirectoryValue)
	})
...
func getLocalGitData() (localGitData, error) {
	...
	slog.Debug("civisibility.git: getting the repository URL")
	...
	slog.Debug("civisibility.git: getting the current branch name")
...
func GetLastLocalGitCommitShas() []string {
	...
	slog.Debug("civisibility.git: getting the commit SHAs of the last 1000 commits in the local Git repository")
...
func UnshallowGitRepository() (bool, error) {
	...
	if !isAShallowClone {
		slog.Debug("civisibility.unshallow: the repository is not a shallow clone")
		return false, nil
	}
	...
	if hasMoreThanOneCommits {
		slog.Debug("civisibility.unshallow: the git log has more than one commits")
		return false, nil
	}
	...
	if major < 2 || (major == 2 && minor < 27) {
		slog.Debug("civisibility.unshallow: the git version is less than 2.27.0 we cannot unshallow the repository")
		return false, nil
	}
	...
	if err != nil || fetchOutput == "" {
		slog.Debug("civisibility.unshallow: error fetching the missing commits and trees from the last month", "error", err.Error())
		// ***
		...
		slog.Debug("civisibility.unshallow: getting the remote branch name")
...
func GetGitDiff(baseCommit, headCommit string) (string, error) {
	...
	if len(baseCommit) != 40 {
		...
		if !re.MatchString(baseCommit) {
			...
			remoteOut, err := execGitString("remote", "show")
			if err != nil {
				slog.Debug("civisibility.git: error on git remote show origin", "remoteOut", remoteOut, "error", err.Error())
			}
			if remoteOut == "" {
				remoteOut = "origin"
			}
...
func CreatePackFiles(commitsToInclude []string, commitsToExclude []string) []string {
	...
	if len(objectsShas) == 0 {
		slog.Debug("civisibility: no objects found to send")
		return nil
	}
...
func detectDefaultBranch(remoteName string) (string, error) {
	...
	defaultRef, err := execGitString("symbolic-ref", "--quiet", "--short", "refs/remotes/"+remoteName+"/HEAD")
	if err == nil && defaultRef != "" {
		...
		if defaultBranch != "" {
			slog.Debug("civisibility.git: detected default branch from symbolic-ref", "defaultBranch", defaultBranch)
			return defaultBranch, nil
		}
	}

	slog.Debug("civisibility.git: could not get symbolic-ref, trying to find a fallback (main, master)...")
...
func findFallbackDefaultBranch(remoteName string) string {
	...
	for _, fallback := range fallbackBranches {
		...
		_, err := execGitString("show-ref", "--verify", "--quiet", "refs/remotes/"+remoteName+"/"+fallback)
		if err == nil {
			slog.Debug("civisibility.git: found fallback default branch", "fallback", fallback)
			return fallback
		}
	}
...
func GetBaseBranchSha(defaultBranch string) (string, error) {
	...
	if err != nil {
		...
		slog.Debug("civisibility.git: failed to detect default branch, using fallback", "defaultBranch", defaultBranch)
	...
	if gitPrBaseBranch != "" {
		...
		slog.Debug("civisibility.git: using git.pull_request.base_branch from CI", "gitPrBaseBranch", gitPrBaseBranch)
	...
	if len(candidateBranches) == 1 {
		...
		baseSha, err := execGitString("merge-base", candidateBranches[0], sourceBranch)
		if err != nil {
...
func getSourceBranch() (string, error) {
	return execGitString("rev-parse", "--abbrev-ref", "HEAD")
}
...
func checkAndFetchBranch(branch, remoteName string) {
	...
	_, err := execGitString("show-ref", "--verify", "--quiet", "refs/remotes/"+remoteName+"/"+branch)
	if err == nil {
		return // branch exists locally
	}

	// Check if branch exists in remote
	remoteHeads, err := execGitString("ls-remote", "--heads", remoteName, branch)
	...
	_, err = execGitString("fetch", "--depth", "1", remoteName, branch)
	if err != nil {
		slog.Debug("civisibility.git: failed to fetch branch", "branch", branch, "error", err.Error())
	}
}
...
func getRemoteBranches(remoteName string) ([]string, error) {
	...
	remoteOut, err := execGitString("for-each-ref", "--format=%(refname:short)", "refs/remotes/"+remoteName)
	if err != nil {
...
func computeBranchMetrics(candidates []string, sourceBranch string) (map[string]branchMetrics, error) {
	...
	for _, candidate := range candidates {
		...
		baseSha, err := execGitString("merge-base", candidate, sourceBranch)
		if err != nil || baseSha == "" {
			continue
		}

		// Count commits ahead/behind
		counts, err := execGitString("rev-list", "--left-right", "--count", candidate+"..."+sourceBranch)
		if err != nil {
			continue
		}
