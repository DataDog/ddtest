...
| --- | --- | --- | --- | --- |
| `cli.command` | count | command | `command`, `exit_code`, `platform`, `framework`, `test_skipping_mode` | Number of completed top-level ddtest commands. `command` is `plan` or `run`; `exit_code` is `0` or `1`; the remaining tags contain the resolved CLI configuration. |
| `cli.command_ms` | distribution | milliseconds | `command`, `exit_code`, `platform`, `framework`, `test_skipping_mode` | Duration of a top-level ddtest command, tagged by command, exit code, and resolved CLI configuration. |
